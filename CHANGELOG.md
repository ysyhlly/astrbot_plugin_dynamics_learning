...
- 抽离评测上下文与参数影响计划；修正 Pair Accuracy 和零 F1，验证证据绑定指标版本 2。
- 样本 schema 4 分开事件、标注、导入时间及标签来源；新鲜度按近期有效数量检查，新增独立消息计数。

- 候选生成与最终验证分离，提前隔离最终会话/时间留出集；关闭调参也执行统一门槛。
- 内容指纹绑定数据、冻结参数、基线和切分；读取宿主有效基线，无法读取时明确默认参考来源。
- Shadow 按完整实验身份独立评估；旧记录仅用于总览。
- 确认空标注同步清理样本，数据变化使旧证据过期；策略写入增加版本冲突检测和发布恢复日志。
- 修复写后刷新旧请求、模型复盘缓存身份与生命周期；查询缓存和工作线程减少重复计算，导入批量更新索引。
- 增加未经补丁宿主发行版契约检查；诊断导出明确不支持恢复。验证边界见 `docs/audit-validation-2026-09-15.md`。

## v1.4.0 — 会话优先学习工作台（2026-09-15）

- 重写为工作台、数据与复盘、评测、影子观察、策略五个栏目，保留已有功能。
- 会话列表联动画像与分页样本证据，支持搜索、筛选、URL 恢复和返回焦点。
- 全局结果与会话范围分开呈现，综合采纳结论优先于单次留出集结果。
- 模型复盘仅由主动操作触发；局部失败保留结果，过期响应不覆盖新数据。
- 脚本按职责拆分，支持宿主签名资源、明暗主题与窄屏逐层导航。
- 新增合成数据预览工具及真实 SDK 资源服务回归。
- 验证：本地 391 项 Python 测试、Node 回归、Ruff 与宽窄屏浏览器检查通过；尚未连接运行中的 AstrBot 完成全流程联调。

## v1.3.2 — 审查修复（2026-09-15）

- 修正回复准入候选参数联动；缺失分数不参与可回放指标、置信区间和拟合。
- 修正调参置信度的样本口径与状态历史；旧重复特征模型对齐时合并权重。
- 分析计算移至工作线程；清空数据同时清除契约快照与复盘缓存。
- 保留宿主声明的结果阶段与来源；未知运行快照版本增加诊断并按结构读取。
- 缺失窗口限额、时钟或标注来源时明确显示未知。
- 修复页面操作、分页、请求超时、缓存、局部失败及空值显示。
- 配套宿主补丁保留 shadow 的 recorded_at；仅修改本工作区宿主副本。

## v1.3.1 — 报出「有多少标签来自模型草稿」

本体 v1.9.0 起可以先用模型起草回复标签、由人采纳后保存，采纳的记录带 `accepted_from: "ai"`。
学习层不替读者决定这算不算人工真值 —— 但必须让人看得见：这一版把它计数并写进契约面发现里。

- 契约面新增 `ai_assisted`（采纳了模型草稿的标注数）与 `human_only`（纯人工），由导入时对原始记录计数；
- `contract_findings` 多一行：「N 条标注是先由模型起草、再由人工采纳的（纯人工 M 条）：这些标签的值来自模型，
  只有「按下保存」这一步是人做的」——按人工真值解释它们之前，先知道这一点；
- 门槛与指标不变：草稿本身永远不进入样本（它写在本体自己的键里），进得来的都是人保存过的记录，
  现在只是把成分标出来。
- 新增 3 项测试：发现的措辞与两个计数、没有草稿采纳时不提这件事、原始记录计数从 `accepted_from` 来。

## v1.3.0 — 待标注窗口：能标哪些、还剩多久

本体的消息图按条数与保留期限制（默认 500 条 / 60 分钟），能人工标注的只有还留在图里的消息；
标注记录本身是永久的，正文也随标注留在本体侧。也就是说，窗口限制的是**标注**，不是数据 —— 但以前
没人把这件事说出来，读者只能在发现「回放页里那条消息不见了」之后才知道。

### 窗口事实

- 新增 `core/window.py` 与只读接口 `GET annotation_window`（`?export=1` 附带消息明细）。
  它读两处：本体的运行快照与标注键，**不调用模型、不写任何数据**。
- 每个会话给出：窗口内条数、带正文条数、已标注 / 未标注条数、窗口跨度、最旧一条距离被清理还有多久，
  以及本体报出的保留规则（快照里没有该字段时才退回默认值，并在面板上标明「按默认值估」）。
- 时间换算用快照自带的 `saved_wall` / `saved_clock` 这一对读数，把节点的单调时钟放到民用时钟上；
  快照没带这对读数时如实显示「—」，不猜。
- 面板新增「待标注窗口」一节，随页面刷新加载（无模型调用）：表格 + 一句结论 + 三张统计卡。

### 导出

- 「导出这段窗口」把当前窗口整段下载成 JSON：消息 id、时间、脱敏发送者、截断 300 字的正文、是否已标注。
  组装在服务端、下载在你本机 —— 插件不保存正文，也不写回本体任何数据。

### 移除空转的 `learning_keep_message_text`

- 这个键承诺「保存消息正文」，但没有任何代码读它（ingest 从不复制正文）。现在删除，并写明实际语义：
  正文随本体的标注记录永久保留，需要读的地方（回复复盘）实时读一次、不落盘。
  已有配置里残留的值会被忽略，没有迁移动作。

### 依赖

- 面板的「本体保留规则」读的是运行快照里的 `graph` 字段，本体 **v1.8.1** 起才有；
  更早的本体上会退回默认的 500 条 / 60 分钟并标明是估算。

### 验证

- 新增 `tests/test_annotation_window.py`（11 项）：窗口计数与截止时间换算、已标注与未标注的区分、
  全部标完的措辞、超出保留期的措辞、本体不报上限时的回落、快照缺时钟对时不猜时间、空快照、
  导出的正文截断与身份脱敏、明细只在需要时附带、插件读取不写任何键、本体读不到时不抛异常。
