...
| `samples` | GET | 分页样本（`page` / `page_size` / `task` / `session`） |
...
```powershell
python -m pytest -q          # 94 项：契约、样本、指标、策略、学习者、评测、插件面
python -m ruff check .
python -m mypy
...
| v0.6 | Group Dynamics Profile（每个群自己的小性格） | 计划 |
| v0.7 | Participation Learning（什么时候该闭嘴） | 计划 |
| v0.8 | Semi Auto Tune（人工一键应用，需要本体开放写入） | 计划 |
| v1.0 | Safe Adaptive Dynamics | 计划 |
