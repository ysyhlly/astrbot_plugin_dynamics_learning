...
发给模型的只有：正文、这条消息有没有提到机器人、以及匿名的会话分组（`c1` / `c2`，同一段对话的消息
能对上、但看不出是哪个群）。**不给人工标注，不给本体的判定，不给实际结果** —— 一个先看过答案的评审
只会同意答案。
...
| 对照 | 模型 vs 实际（漏回 / 多回 / 一致），以及模型 vs 人工（一致 / 不一致）|

三列不一致的那几行才是这张表的意义。「模型认为漏回 3 条」比任何总体准确率都更容易去查。
